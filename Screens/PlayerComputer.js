...
const PlayerComputer = () => {
  ...
  const autoCompleteTime = 2; // seconds for automatic completion of Player 2's turn
  ...
  const makeComputerMove = () => {
      ...
      if (availableLines.length > 0) {
        ...
        const randomLine = availableLines[randomIndex];
        handleLineDraw(randomLine);
      }
  };

  const getAllAvailableLines = () => {
    const availableLines = [];
    for (let i = 0; i < dots.length; i++) {
      for (let j = i + 1; j < dots.length; j++) {
        const startDot = dots[i];
        const endDot = dots[j];
        if (areAdjacent(startDot, endDot) && !isLineDrawn(startDot, endDot)) {
          availableLines.push({ startDot, endDot });
        }
      }
    }
  ...
  const isLineDrawn = (startDot, endDot) => {
    return lines.some(
      (line) =>
        (line.startDot.id === startDot.id && line.endDot.id === endDot.id) ||
        (line.startDot.id === endDot.id && line.endDot.id === startDot.id)
    );
  };

  const handleLineDraw = (line) => {
    const newLine = {
      startDot: {
        ...line.startDot,
        row: parseInt(line.startDot.id.split("-")[0]),
        col: parseInt(line.startDot.id.split("-")[1]),
      },
      endDot: {
        ...line.endDot,
        row: parseInt(line.endDot.id.split("-")[0]),
        col: parseInt(line.endDot.id.split("-")[1]),
      },
  ...
  const checkForSquare = (line) => {
    ...
    if (squareCount > 0) {
      if (line.player === 1) {
        setScore1((prevScore) => prevScore + squareCount);
      } else {
        setScore2((prevScore) => prevScore + squareCount);
      }
    }
  };
  const winner = () => {
